...
class MFCent(object):
    ...
    def fit(self, X, y):
        ...
        #
        self.docs_by_class = [len(np.where(self.y_train == i)[0])
                              for i in self.classes]

        #
        self.centroids = [np.mean(self.X_train[np.where(
            self.y_train == i)], axis=0, dtype=np.float64) for i in self.classes]

        #
        self.centroids_sum = [np.sum(self.X_train[np.where(
            self.y_train == i)], axis=0, dtype=np.float64) for i in self.classes]
