...
class MultiHeadAttentionEncoder(pl.LightningModule):
    ...
    def forward(self, x, attn_y, learned):

        key = self.key_fnn(x)
        query = self.query_fnn(x)
        value = self.value_fnn(x)

        _, attn_weights = self.multihead_att(query, key, value)
        learned_w = torch.mean(attn_weights, dim=1)

        if learned[0] > torch.Tensor([0])[0]:
            weighted_probs = x * learned_w.unsqueeze(-1)
        else:
            upper = torch.mean(attn_y, dim=1)
...
class MetaLayer(pl.LightningModule):
    ...
    def forward(self, x, attn_y, learned):
        ...
        attn_out, _ = self.attn_encoder(x, attn_y, learned)
    ...
    def training_step(self, batch):
        ...
        attn_output, attn_weights = self.attn_encoder(x, attn_y, torch.Tensor([0]))
        ...
        # Computing MLP loss.
        y_hat = self(x, attn_y, torch.Tensor([0]))
